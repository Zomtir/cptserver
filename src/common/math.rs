...
pub fn decode_hash256(hash: &str) -> Result<Vec<u8>> {
    match &hash.len() {
        // Sha256 is 64 chars long
        64 => match hex::decode(hash) {
            Ok(bytes) => Ok(bytes),
            _ => Err(Error::new(ErrorKind::Default, "Hash is not valid hex")),
        },
        _ => Err(Error::new(ErrorKind::Default, "Hash has incorrect length")),
    }
}

pub fn decode_hash128(hash: &str) -> Result<Vec<u8>> {
    match &hash.len() {

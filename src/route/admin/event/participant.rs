...
pub fn registration_list(session: UserSession, event_id: u64) -> Result<Json<Vec<User>>, Error> {
    ...
    let users = crate::db::event::participant::event_participant_registration_list(event_id)?;
    Ok(Json(users))
}

#[rocket::get("/admin/event_participant_filter_list?<event_id>")]
pub fn filter_list(session: UserSession, event_id: u64) -> Result<Json<Vec<(User, bool)>>, Error> {
    if !session.right.right_event_read {
        return Err(Error::RightEventMissing);
    };

    let filters = crate::db::event::participant::event_participant_filter_list(event_id)?;
    Ok(Json(filters))
}

#[rocket::head("/admin/event_participant_filter_edit?<event_id>&<user_id>&<access>")]
pub fn filter_edit(session: UserSession, event_id: u64, user_id: u64, access: bool) -> Result<(), Error> {
    ...
    crate::db::event::participant::event_participant_filter_edit(event_id, user_id, access)?;
    Ok(())
}

#[rocket::head("/admin/event_participant_filter_remove?<event_id>&<user_id>")]
pub fn filter_remove(session: UserSession, event_id: u64, user_id: u64) -> Result<(), Error> {
    if !session.right.right_event_write {
        return Err(Error::RightEventMissing);
    };

    crate::db::event::participant::event_participant_filter_remove(event_id, user_id)?;
    Ok(())
}

#[rocket::get("/admin/event_participant_presence_pool?<event_id>")]
...
#[rocket::get("/admin/event_participant_presence_list?<event_id>")]
...
#[rocket::head("/admin/event_participant_presence_add?<event_id>&<user_id>")]
...
#[rocket::head("/admin/event_participant_presence_remove?<event_id>&<user_id>")]

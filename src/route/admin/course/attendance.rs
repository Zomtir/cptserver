...
pub fn sieve_list(session: UserSession, course_id: u32, role: String) -> Result<Json<Vec<(Team, bool)>>, Error> {
...
pub fn sieve_edit(session: UserSession, course_id: u32, team_id: u64, role: String, access: bool) -> Result<(), Error> {
    let conn = &mut crate::utils::db::get_db_conn()?;
...
pub fn sieve_remove(session: UserSession, course_id: u32, team_id: u64, role: String) -> Result<(), Error> {
    let conn = &mut crate::utils::db::get_db_conn()?;
    if !session.right.right_course_write {
        return Err(Error::RightCourseMissing);
    };

...
pub fn possession_info(conn: &mut PooledConn, possession_id: u64) -> Result<Possession> {
    let stmt = conn.prep(
        "SELECT up.possession_id,
            u.user_id, u.user_key, u.firstname, u.lastname, u.nickname,
            i.item_id, i.name as item_name, ic.category_id, ic.name as category_name,
    ...
    let row: mysql::Row = match conn.exec_first(&stmt, &params)? {
        None => return Err(Error::new(ErrorKind::Missing, "Possession not found")),
        Some(row) => row,
    };
...
pub fn possession_ownership(conn: &mut PooledConn, possession_id: u64) -> Result<Option<Stock>> {
    let stmt = conn.prep(
        "SELECT cs.stock_id,
...
pub fn possession_delete(conn: &mut PooledConn, possession_id: u64) -> Result<()> {
    let stmt = conn.prep(
        "DELETE up
        FROM user_possessions up
        WHERE up.possession_id = :possession_id;",

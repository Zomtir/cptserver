...
pub fn stock_list(conn: &mut PooledConn, club_id: Option<u32>, item_id: Option<u32>) -> Result<Vec<Stock>> {
    let stmt = conn.prep(
        "SELECT
            cs.stock_id,
...
pub fn stock_info(conn: &mut PooledConn, stock_id: u64) -> Result<Stock> {
    let stmt = conn.prep(
        "SELECT
            cs.stock_id,
            c.club_id, c.club_key, c.name as club_name,
    ...
    let mut row: mysql::Row = match conn.exec_first(&stmt, &params)? {
        None => return Err(Error::new(ErrorKind::Missing, "Stock not found")),
        Some(row) => row,
    };
...
pub fn stock_create(conn: &mut PooledConn, club_id: u64, item_id: u64, storage: &str, owned: u32) -> Result<()> {
    let stmt = conn.prep(
...
pub fn stock_edit(conn: &mut PooledConn, stock_id: u64, storage: &str, owned: u32, loaned: u32) -> Result<()> {
    let stmt = conn.prep(
        "UPDATE club_stocks
        SET
            storage = :storage,
...
pub fn stock_delete(conn: &mut PooledConn, stock_id: u64) -> Result<()> {
    let stmt = conn.prep(

...
pub fn competence_info(conn: &mut PooledConn, competence_id: Option<u64>) -> Result<Option<Competence>> {
    let stmt = conn.prep(
        "SELECT uc.competence_id,
            u.user_id, u.user_key, u.firstname as user_firstname, u.lastname as user_lastname, u.nickname as user_nickname,
            s.skill_id, s.skill_key, s.name as skill_name, s.min as skill_min, s.max as skill_max,
...
pub fn competence_edit(conn: &mut PooledConn, competence_id: u64, competence: &Competence) -> Result<()> {
    let stmt = conn.prep(
...
pub fn competence_delete(conn: &mut PooledConn, competence_id: u64) -> Result<()> {

...
pub fn team_member_list(conn: &mut PooledConn, team_id: u32) -> Result<Vec<User>> {
    let stmt = conn.prep(
...
pub fn team_member_add(conn: &mut PooledConn, team_id: &u32, user_id: &u32) -> Result<()> {
...
pub fn team_member_remove(conn: &mut PooledConn, team_id: &u32, user_id: &u32) -> Result<()> {

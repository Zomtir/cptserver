...
pub fn team_list(conn: &mut PooledConn) -> Result<Vec<Team>> {
    let stmt = conn.prep(
        "SELECT
            team_id,
            team_key,
...
pub fn team_info(conn: &mut PooledConn, team_id: &u32) -> Result<Team> {
    let stmt = conn.prep(
        "SELECT
            team_id,
            team_key,
            ...
            right_course_write,
            right_course_read,
            right_discipline_write,
            right_discipline_read,
            right_event_write,
            right_event_read,
    ...
    let mut row: mysql::Row = match conn.exec_first(&stmt, &params)? {
        None => return Err(Error::new(ErrorKind::Missing, "Team not found")),
        Some(row) => row,
    };
    ...
    let team = Team {
        ...
        right: Some(Right {
            ...
            right_discipline_write: row.take("right_discipline_write").unwrap(),
            right_discipline_read: row.take("right_discipline_read").unwrap(),
...
pub fn team_create(conn: &mut PooledConn, team: &Team) -> Result<u32> {
    let stmt = conn.prep(
        "INSERT INTO teams (
            team_key,
            name,
...
pub fn team_edit(conn: &mut PooledConn, team_id: &u32, team: &Team) -> Result<()> {
    let stmt = conn.prep(
        "UPDATE teams SET
            team_key = :team_key,
            name = :name,
...
pub fn team_right_edit(conn: &mut PooledConn, team_id: &u32, right: &Right) -> Result<()> {
    let stmt = conn.prep(
        "UPDATE teams SET
            ...
            right_discipline_write = :right_discipline_write,
            right_discipline_read = :right_discipline_read,
    ...
    let params = params! {
        ...
        "right_discipline_write" => &right.right_discipline_write,
        "right_discipline_read" => &right.right_discipline_read,
...
pub fn team_delete(conn: &mut PooledConn, team_id: &u32) -> Result<()> {

...
ALTER TABLE `organisation_affiliations` DROP INDEX `organisation_members_ibfk_1`;
ALTER TABLE `organisation_affiliations` DROP INDEX `organisation_members_ibfk_2`;
ALTER TABLE `organisation_affiliations` ADD PRIMARY KEY (`organisation_id`, `user_id`) USING BTREE;
...
ALTER TABLE `term_disciplines` ADD CONSTRAINT `term_disciplines_ibfk_1` FOREIGN KEY (`term_id`) REFERENCES `terms`(`term_id`) ON UPDATE CASCADE;
ALTER TABLE `term_disciplines` ADD CONSTRAINT `term_disciplines_ibfk_2` FOREIGN KEY (`discipline_id`) REFERENCES `disciplines`(`discipline_id`) ON UPDATE CASCADE;
...
ALTER TABLE `skills` CHANGE `title` `name` TINYTEXT NOT NULL;

-- Clean up redundant indexes
ALTER TABLE `event_owners` DROP INDEX `REF_user`;
ALTER TABLE `event_attendance_filters` DROP INDEX `REF_user`;
ALTER TABLE `event_attendance_presences` DROP INDEX `REF_user`;
...
ALTER TABLE `user_equipment` ADD CONSTRAINT `user_equipment_ibfk_1` FOREIGN KEY (`user_id`) REFERENCES `users`(`user_id`) ON UPDATE CASCADE;
ALTER TABLE `user_equipment` ADD CONSTRAINT `user_equipment_ibfk_2` FOREIGN KEY (`skill_id`) REFERENCES `skills`(`skill_id`) ON UPDATE CASCADE;
ALTER TABLE `user_equipment` ADD CONSTRAINT `user_equipment_ibfk_3` FOREIGN KEY (`item_id`) REFERENCES `items`(`item_id`) ON UPDATE CASCADE;
